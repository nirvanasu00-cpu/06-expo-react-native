...
sequenceDiagram
    ...
    Comp->>Effect: mount (依赖 [])
    Effect-->>Comp: setTimeout 模拟数据加载
    Comp->>State: setTodos([...])
    State-->>Comp: 新状态
    Comp->>Render: 重新渲染

    Note over Comp,Render: 用户输入文字
    Comp->>State: setInput('文字')
    State-->>Comp: 新输入值
    Render-->>Comp: 输入框更新

    Note over Comp,Render: 用户点击添加
    Comp->>State: setTodos(prev => [...prev, new])
    State-->>Comp: 新 todos
    Comp->>Effect: todos 变化触发 effect
    Effect-->>Comp: console.log 日志

    Note over Comp,Render: 组件卸载
    Comp->>Effect: 清理函数 clearTimeout
```
...
classDiagram
    class useState {
        +initialValue: T
        +value: T
        +setValue(newValue or updater)
        +functionalUpdate(prev => next)
    }
    class useEffect {
        +effect: () => (() => void)?
        +dependencies: any[]?
    ...
    useState --> useEffect : 作为依赖
    useEffect --> useCallback : 用于性能优化
    useMemo --> useCallback : 底层实现关联
```
